...
func (fr *FlightRepo) GetBy(placeSource string, placeDestination string, dateDestination string) ([]*model.Flight, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	layout := "2006-01-02"
	parsedDate, err := time.Parse(layout, dateDestination)
	if err != nil {
		fr.logger.Println(err)
		return nil, err
	}
	...
	flightsCursor, err := fr.db.Find(ctx, bson.M{
		"placeSource":      bson.M{"$regex": placeSource, "$options": "i"},
		"placeDestination": bson.M{"$regex": placeDestination, "$options": "i"},
		"dateDestination":  bson.M{"$gte": parsedDate, "$lt": nextDay},
	})
	if err != nil {
		fr.logger.Println(err)
		return nil, err
